...
start_url <- 'https://genius.com/albums/6ix9ine/'
other_url <- "https://genius.com/6ix9ine"

tekashi_scrape <- function(album){
  song_titles <- list()
  album_titles <- list()
  lyrics <- list()
  ...
  for(h in html_search_terms){
    ...
    for(l_url in l){
      if(l_url == "https://genius.com/6ix9ine-chocolaté-lyrics"){
        l_url = "https://genius.com/6ix9ine-chocolate-lyrics"
      }
...
scummy <- tekashi_scrape("Scummy-scumz")
#day69 <- tekashi_scrape("Day69") ~~~!!!extraneous!!!~~~
grad <- tekashi_scrape("Day69-graduation-day")
dummy_boy <- tekashi_scrape("Dummy-boy")

tekashi_discography <-  do.call("rbind", list(scummy, grad, dummy_boy))


write.csv(tekashi_discography, "/Users/ericbrigham/Desktop/datamining/project/tekashilyrics.csv")
